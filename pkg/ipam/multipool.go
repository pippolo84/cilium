...
func (m *multiPoolManager) updateLocalNodeStore(newNode *ciliumv2.CiliumNode) {
	...
	// Skip the update if primary alloc CIDRs are nil.
	// This may happen during startup, when no pools have been allocated yet.
	// In that case, leave the default IPv4/IPv6 allocation CIDR derived from
	// DefaultIPv4Prefix and IPv6ClusterAllocCIDR, respectively.
