...
func (b *BGPResourceManager) getMatchingNodes(nodeSelector *slim_meta_v1.LabelSelector, configName string) (map[string]struct{}, error) {
	...
	matchingNodes := make(map[string]struct{})
...
func (b *BGPResourceManager) deleteStaleNodeConfigs(ctx context.Context, expectedNodes map[string]struct{}, clusterRef string) error {
	var err error
	for _, existingNode := range b.nodeConfigStore.List() {
		_, exists := expectedNodes[existingNode.Name]
		if exists || !IsOwner(existingNode.GetOwnerReferences(), clusterRef) {
			continue
		}
