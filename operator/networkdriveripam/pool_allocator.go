...
func NewPoolAllocator(logger *slog.Logger, config *option.DaemonConfig) *PoolAllocator {
	return &PoolAllocator{
		logger:      logger,
		ipv4Enabled: config.EnableIPv4,
...
func (p *PoolAllocator) upsertPool(ctx context.Context, ipPool *cilium_v2alpha1.CiliumResourceIPPool) error {
	var ipv4CIDRs, ipv6CIDRs []string
	var ipv4MaskSize, ipv6MaskSize int

	if ipPool.Spec.IPv4 != nil {
		ipv4MaskSize = int(ipPool.Spec.IPv4.MaskSize)
		ipv4CIDRs = make([]string, len(ipPool.Spec.IPv4.CIDRs))
		for i, cidr := range ipPool.Spec.IPv4.CIDRs {
			ipv4CIDRs[i] = string(cidr)
		}
	}

	if ipPool.Spec.IPv6 != nil {
		ipv6MaskSize = int(ipPool.Spec.IPv6.MaskSize)
		ipv6CIDRs = make([]string, len(ipPool.Spec.IPv6.CIDRs))
		for i, cidr := range ipPool.Spec.IPv6.CIDRs {
	...
	p.logger.Debug(
		"upserting pool",
		logfields.PoolName, ipPool.Name,
	...
	pool, exists := p.pools[ipPool.Name]
	if exists && ipv4MaskSize != pool.v4MaskSize {
		return fmt.Errorf("cannot change IPv4 mask size in existing pool %q", ipPool.Name)
	}
	if exists && ipv6MaskSize != pool.v6MaskSize {
		return fmt.Errorf("cannot change IPv6 mask size in existing pool %q", ipPool.Name)
	}
	...
	return p.reconcileOrphanCIDRs(ipPool.Name, v4, v6)
}

func (p *PoolAllocator) deletePool(ctx context.Context, pool *cilium_v2alpha1.CiliumResourceIPPool) error {
	p.logger.Debug(
		"deleting pool",
	...
	if _, exists := p.pools[pool.Name]; !exists {
		return fmt.Errorf("pool %q requested for deletion doesn't exist", pool.Name)
	}

	for node, pools := range p.nodes {
		cidrSets, found := pools[pool.Name]
		if !found {
			continue
		}
		p.logger.Warn(
		...
		delete(p.nodes[node], pool.Name)
		for cidr := range cidrSets.v4 {
			p.markOrphan(node, pool.Name, cidr)
		}
...
func (p *PoolAllocator) AllocateToNode(cn *v2.CiliumNode) error {
	...
	allocatedSet := make(map[string]map[netip.Prefix]struct{}, len(cn.Spec.IPAM.ResourcePools.Allocated))
	for _, allocatedPool := range cn.Spec.IPAM.ResourcePools.Allocated {
	...
	for _, reqPool := range cn.Spec.IPAM.ResourcePools.Requested {
